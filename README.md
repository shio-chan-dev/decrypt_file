...
如果脚本提示 `NVIDIA driver on your system is too old`，通常表示当前 Python 环境里的 PyTorch CUDA 轮子版本高于服务器驱动支持的版本。本项目通过 uv 将 GPU 可选依赖中的 `torch` 固定到 PyTorch `cu126` 源，CUDA 12.6 环境可执行 `uv sync --extra gpu` 后重新检查。
